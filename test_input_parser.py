...
class Tests(TestCase):
    ...
    def test_get_expected_data_type(self):
        self.assertEqual(get_expected_data_type(1), 'digits')
        self.assertEqual(get_expected_data_type("AAA "), 'word_characters')
        self.assertEqual(get_expected_data_type("-AA"), 'others')
        self.assertEqual(get_expected_data_type("-"), 'others')
    
    def test_is_expected_max_length(self):
        self.assertTrue(is_expected_max_length(2, "AS"))
        self.assertFalse(is_expected_max_length(2, "S"))
        self.assertTrue(is_expected_max_length(2, 12))

    def test_process_subsections(self):
        report = []
        messages_list = []
        sub_sections = self.expected_key_index_dict['L1']['sub_sections']
        row = (0,pd.Series(["L1", 2], index =['Sections', "1"]))
        process_subsections(row, "L1", sub_sections, report, self.error_codes, messages_list)
